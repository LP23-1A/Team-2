...
const SignPage = () => {
  ...
  const nextStep = () => {
    const { email, name, phoneNumber } = formDataRef.current;
    switch (true) {
      case !email:
  ...
  const nextStep2 = () => {
    const { password, repassword } = formDataRef.current;
    switch (true) {
      case !password:
        setError("password is required");
        break;
      case !repassword:
        setError("repassword is required");
        break;
      default:
        setError("");
        setActiveStep((prevStep) =>
  ...
  return (
    <>
      ...
      {activeStep === 0 && <button onClick={nextStep2}></button>}
      {activeStep !== 0 && activeStep !== steps.length - 1 && (
        <>
          <button className="" onClick={prevStep}></button>
          <button onClick={nextStep2}></button>
        </>
      )}
    </>
  );
