...
export default function ContactInformation({ data }: any) {
  ...
  const orderProduct = async () => {
    if (
      ...
      !formDataRef.current.phoneNumber ||
      !formDataRef.current.firstName ||
      !formDataRef.current.lastName ||
      !formDataRef.current.city ||
      !formDataRef.current.appaetnentment ||
      !formDataRef.current.address ||
      !formDataRef.current.city ||
      !formDataRef.current.postalCode
    ) {
      setError("Бөглөөгүй хэсгийг бөглөнө үү");
      return;
    }
    try {
        ...
        <div className="flex justify-between ">
            ...
            <div className="py-[50px] px-[30px]">
              ...
              <div className="mt-[40px] ">
                <input
                  ...
                  onChange={(e) =>
                    handleOnChange("Appaetnentment", e.target.value)
                  }
                />
                <div className="mt-[10px]">
                  <hr />
              ...
              <div>
                <button className="py-[10px] px-[15px] text-[white] bg-[#FB2E86] mt-[80px] rounded-[5px] font-[600]">
                  Continue Shipping
                </button>
              </div>
            </div>
          ...
          <div className="w-[384px] mt-[30px]">
              ...
              data.map((product): any => {
                return (
                  <section className="flex items-center justify-between mt-[20px]">
                    <div>
                      <img
                        className="w-[86px] h-[87px] rounded-md"
                        src={product?._id?.images}
                        alt=""
                      />
                    </div>
                    ...
                    <div>{product?._id?.price * product?.quantity}</div>
                  </section>
                );
              })}
            ...
            <div className="384px p-[30px] bg-[#F8F8FD] rounded-md mt-[30px]">
              ...
              <button className="w-full py-[10px] px-[20px] mt-[20px] bg-[#19D16F] rounded-md text-[white]">
                Худалдан авах
              </button>
              <div>
                {error && (
                  <p className=" text-[#ff0000] font-extrabold text-center">
                    {error}
                  </p>
                )}
              </div>
